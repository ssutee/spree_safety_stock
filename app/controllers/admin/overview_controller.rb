...
class Admin::OverviewController < Admin::BaseController
  ...
  def index
    ...
    @over_safety_stock_products = over_safety_stock_products
    @out_of_stock_variants = out_of_stock_variants
    @over_safety_stock_variants = over_safety_stock_variants    
  ...
  def over_safety_stock_products
    Product.where("count_on_hand <= safety_stock").select { |product| product.variants.count == 0 }
  end
  
  
  def over_safety_stock_variants
    Variant.where("count_on_hand <= safety_stock").select { |variant| variant.is_master == false }
  end
